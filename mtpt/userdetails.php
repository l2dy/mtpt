...
if (get_user_class() >= $userprofile_class) {
	...
	if ($iphistory > 0)
	tr_small($lang_userdetails['row_ip_history'], $lang_userdetails['text_user_earlier_used']."<b><a href=\"iphistory.php?id=" . $user['id'] . "\" id='ip'>" . $iphistory. $lang_userdetails['text_different_ips'].add_s($iphistory, true)."</a></b>", 1);
...
if (get_user_class() >= $userprofile_class ||  $user["id"] == $CURUSER["id"])
{
	...
	if($user['cardnum'] == null) $cardstatus="无";
	if($user['cardnum'] != null){
		if($user['cardconfirm'] == 'yes'){
			$cardstatus="已验证";
		}elseif($user['cardconfirm'] == 'no'){
			$cardstatus="未验证";
		}else{
			$cardstatus="未知";
		}
	}
	tr_small("学号状态", $cardstatus, 1);
}
