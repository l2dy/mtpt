...
if ($action == 'savesettings_main')	// save main
{
	...
	$validConfig = array('site_online','max_torrent_size','announce_interval', 'annintertwoage', 'annintertwo', 'anninterthreeage', 'anninterthree', 'signup_timeout','minoffervotes','offervotetimeout', 'edureg', 'offeruptimeout','maxsubsize','postsperpage', 'topicsperpage', 'torrentsperpage', 'maxnewsnum','max_dead_torrent_time','maxusers','torrent_dir', 'iniupload','iniseedbonus','SITEEMAIL', 'ACCOUNTANTID', 'ALIPAYACCOUNT', 'PAYPALACCOUNT', 'SLOGAN', 'icplicense', 'autoclean_interval_one', 'autoclean_interval_two', 'autoclean_interval_three','autoclean_interval_four', 'autoclean_interval_five','reportemail','invitesystem','loginadd','registration','cardreg','delaycardcheck','showhotmovies','showclassicmovies','showimdbinfo', 'enablenfo', 'enableschool','restrictemail','showpolls','showstats','showlastxtorrents', 'showtrackerload','showshoutbox','showfunbox','showoffer','sptime','showhelpbox','enablebitbucket', 'smalldescription','altname','extforum','extforumurl','defaultlang','defstylesheet', 'donation','spsct','browsecat','specialcat','waitsystem','maxdlsystem','bitbucket','torrentnameprefix', 'showforumstats','verification','invite_count','invite_timeout', 'seeding_leeching_time_calc_start','startsubid', 'logo');
	GetVar($validConfig);
	unset($MAIN);
...
elseif ($action == 'bonussettings'){
	...
	tr($lang_settings['row_one_gb_credit_down'],$lang_settings['text_it_costs_user']."<input type='text' style=\"width: 50px\" name=onegbdownload value='".(isset($BONUS["onegbdownload"]) ? $BONUS["onegbdownload"] : 300 )."'>".$lang_settings['text_one_gb_credit_note_down'], 1);
	tr($lang_settings['row_five_gb_credit_down'],$lang_settings['text_it_costs_user']."<input type='text' style=\"width: 50px\" name=fivegbdownload value='".(isset($BONUS["fivegbdownload"]) ? $BONUS["fivegbdownload"] : 800 )."'>".$lang_settings['text_five_gb_credit_note_down'], 1);
	tr($lang_settings['row_ten_gb_credit_down'],$lang_settings['text_it_costs_user']."<input type='text' style=\"width: 50px\" name=tengbdownload value='".(isset($BONUS["tengbdownload"]) ? $BONUS["tengbdownload"] : 1200 )."'>".$lang_settings['text_ten_gb_credit_note_down'], 1);
...
elseif ($action == 'mainsettings')	// main settings
{
    ...
    yesorno('校内用户延迟验证', 'delaycardcheck', $MAIN['delaycardcheck'],"请保证开启校内用户注册再开启此项，此项为校内IP开放注册功能，校内用户可延迟验证学号,默认'否'");
    yesorno('教育网邮箱自助注册', 'edureg', $MAIN['edureg'],"允许教育网邮箱自助注册,默认'否'");
