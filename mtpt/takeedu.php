...
require_once(get_langfile_path("takesignup.php",false));
cur_user_check();
...
function isproxy() {
    $ports = array(80, 88, 1075, 1080, 1180,
        1182, 2282, 3128, 3332, 5490,
        6588, 7033, 7441, 8000, 8080,
        8085, 8090, 8095, 8100, 8105,
        8110, 8888, 22788
    );
...
function EmailIsEduOrAc($value) {
    $email_domains = array(".edu.cn", ".ac.cn");
    foreach ($email_domains as $item)
        if (strEndsWith($value, $item)) return true;
    return false;
}
...
if (!EmailIsEduOrAc($email))
    bark('只允许以 .edu.cn 或 .ac.cn 结尾的邮箱通过本通道注册。');

if (strEndsWith($email, '@nwsuaf.edu.cn') || strEndsWith($email, '@nwafu.edu.cn'))
    bark('西农用户请通过校内用户注册通道注册。');
...
$ret = sql_query("INSERT INTO users (username, passhash, secret, editsecret, email, country, gender, status, class, invites, " . ($type == 'invite' ? "invited_by," : "") . " added, last_access, lang, stylesheet" . ($showschool == 'yes' ? ", school" : "") . ", uploaded,ip,cardnum,cardconfirm) VALUES (" . $wantusername . "," . $wantpasshash . "," . $secret . "," . $editsecret . "," . $email . "," . $country . "," . $gender . ", 'pending', " . $defaultclass_class . "," . $invite_count . ", " . ($type == 'invite' ? "'$inviter'," : "") . " '" . date("Y-m-d H:i:s") . "' , " . " '" . date("Y-m-d H:i:s") . "' , " . $sitelangid . "," . $defcss . ($showschool == 'yes' ? "," . $school : "") . "," . ($iniupload_main > 0 ? $iniupload_main : 0) . ",'" . getip() . "',$stuid" . ",'" . $cardconfirm . "')") or sqlerr(__FILE__, __LINE__);
$id      = mysql_insert_id();
