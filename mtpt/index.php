...
//此处原来的findip函数不对，移到了functions_smarty.php并重写了。ip检测不应该只在index，ip判断移到了header中，保证在所有页面都可以判断ip。By纳兰斯坦2017.04.29
...
$Cache->new_page('recent_news', 43200, true);
if (!$Cache->get_page()){
	...
	foreach($type as $type_each)
	{
			...
			if(mysql_num_rows($result) != 0 )
			{
				while( $row = mysql_fetch_assoc($result) )
				{
					...
					if($picrow['thumb'] == "1")
					print ("<a  target=\"_blank\" class=\"theme\" href=\"details.php?id=". $row['id'] ."&amp;hit=1\" title=\"".htmlspecialchars($row['name'])."\"><img src=\"attachments/".$picrow['location'].".thumb.jpg\" height=120px width=100px></a>");
					else
					print ("<a target=\"_blank\"  class=\"theme\" href=\"details.php?id=". $row['id'] ."&amp;hit=1\" title=\"".htmlspecialchars($row['name'])."\"><img src=\"attachments/".$picrow['location']."\" height=120px width=100px></a>");
					$countnum++;
				}
			}
	}
	if(!$countnum)
		print ("<a target=\"_blank\" class=\"theme\" href=\"#\"><img src=\"pic/imdb_pic/nophoto.gif\" height=120px width=100px></a>");
	print("</div>");
	print("</div>");
...
if ($showlastxtorrents_main == "yes") {
		...
		if(mysql_num_rows($result) != 0 )
		{
			...
			while( $row = mysql_fetch_assoc($result) )
			{
				print ("<tr><a target=\"_blank\" href=\"details.php?id=". $row['id'] ."&amp;hit=1\"><td><a  target=\"_blank\" href=\"details.php?id=". $row['id'] ."&amp;hit=1\"><b>" . htmlspecialchars($row['name']) . "</b></td></a><td align=\"center\">" . $row['seeders'] . "</td><td align=\"center\">" . $row['leechers'] . "</td></tr>");
			}
			print ("</table>");
		}
}
