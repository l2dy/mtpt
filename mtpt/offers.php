...
<?
	...
	"<tr><td class=rowhead align=right><b>子类型<font color=red>*</font></b></td><td class=rowfollow align=left><select id='source_sel' name='source_sel'></select></td></tr>".
...
if ($_GET["new_offer"]){
	...
	if (!$arr['name']){
		...
		write_log("候选：用户 $CURUSER[username] 发布了候选 $name ",'normal');
...
if ($_GET["allow_offer"]) {
	...
	write_log("候选：管理员 $CURUSER[username] 允许了候选  $arr[name]",'normal');
...
if ($_GET["finish_offer"]) {
	...
	write_log("候选：管理员 $CURUSER[username] 关闭了候选 $arr[name]",'normal');
...
<?
	...
	tr("子类型<font color=\"red\">*</font>", "<select id='source_sel' name='source_sel'></select>", 1);
...
if ($_GET["take_off_edit"]){
	...
	write_log("候选：".($CURUSER[id] == $num[userid]?"发布者":"管理员")." $CURUSER[username] 编辑了候选 $name ",'normal');
	//header("Refresh: 0; url=offers.php?id=$id&off_details=1");
}
//======end take offer edit
	...
	if ($vote =='yeah' || $vote =='against')
	{
		...
		else
		{
			...
			if(($yeah-$against)>=$minoffervotes && $ya_arr['allowed'] != "allowed")
			{
				...
				write_log("候选：系统 允许了候选 $arr[name]",'normal');
			}
			...
			if(($against-$yeah)>=$minoffervotes && $ya_arr['allowed'] != "denied")
			{
				...
				sql_query("INSERT INTO messages (sender, receiver, added, msg, subject) VALUES(0, $arr[userid], " . sqlesc(date("Y-m-d H:i:s")) . ", " . sqlesc($msg) . ", ".sqlesc($subject).")") or sqlerr(__FILE__, __LINE__);
				write_log("候选：系统拒绝了候选 $arr[name]",'normal');
			}
...
if ($_GET["del_offer"]){
	...
	elseif ($sure == 1)
	{
		...
		write_log("候选：管理员 $CURUSER[username] 删除了候选  $offer ($num[name])  ".($reason != "" ? " 原因是(".$reason.")" : ""),'normal');
