...
<?php
	...
	if (get_user_class()>= $torrentsticky_class || (get_user_class() >= $torrentmanage_class && $CURUSER["class"] >= '13')){
		...
		{
			$pickcontent .="<b>是否MTTV组:</b><input type=\"radio\" name=\"ismttv\" value=\"yes\"/>yes<input type=\"radio\" name=\"ismttv\" value=\"no\" />no 当前为$row[ismttv]<br/>";
			$pickcontent .='<select name="prohibit_reshipment"><option value="no_restrain">无限制</option><option value="cernet_only">仅限教育网转载</option><option value="prohibited">禁止转载</option></select> <b>非MTTV组成员请勿修改此项！</b><br/> <script>$(document).ready(function(){$(\'select[name="prohibit_reshipment"]\').val("'.$row['prohibit_reshipment'].'")})</script>';
			...
			"<option" . (($row["pos_state"] == "sticky1") ? " selected=\"selected\"" : "" ) . " value=\"1\">"."一级置顶"."</option>" .
			"<option" . (($row["pos_state"] == "sticky") ? " selected=\"selected\"" : "" ) . " value=\"2\">"."二级置顶"."</option>" .
