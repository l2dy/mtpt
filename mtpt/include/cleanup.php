...
$lasttime=$memcache->get('drawnumtime');
		...
		{
			$memcache->set('drawnumtime',(int)time(),0,0);
			//echo time();
	...
	function countLogNumber(){
	$res=sql_query('SELECT LogTime FROM everydaycount ORDER BY Id DESC;');
	$num = mysql_num_rows($res);
	if($num != 0){//表中为空就直接下一步吧
		$lastTime = mysql_fetch_row($res);
		$time = date("Y-m-d H:i:s",(time()-43200));
		if($lastTime[0]  > $time){//如果12小时前更新过则退出
			return;
		}
	}//sendMessage(0,34627,'00000',$lastTime[0]."--$time--".date());
	if(((int)date("H")) != 22){//22点到23点之间更新
		return;
	}
	$bonussums = get_row_sum("users","seedbonus");//全站麦粒数
	$invitesums = get_row_sum("users","invites");//全站邀请码数
	$dayLogNum = get_row_count("users","WHERE last_access >= ". sqlesc(date("Y-m-d H:i:s",(TIMENOW - 86400))));
	if(date("w", TIMENOW) == 6  && date("j", TIMENOW) == date("t", date("m", TIMENOW)) ){//一周的最后一天且一个月的最后一天
		$weekLogNum =  get_row_count("users","WHERE last_access >= ". sqlesc(date("Y-m-d H:i:s",(TIMENOW - 3849216))));
		$monthLogNum = get_row_count("users","WHERE last_access >= ". sqlesc(date("Y-m-d H:i:s",(TIMENOW - 86400 *date("t", date("m", TIMENOW) )))));
		sql_query('INSERT INTO everydaycount (LogTime, dayLogNum, weekLogNum, monthLogNum,seedbounsNum, invitesNum)  VALUES (now(), '.$dayLogNum.', '.$weekLogNum.', '.$monthLogNum.', '.$bonussums . ', ' . $invitesums . ')')or sqlerr(__FILE__, __LINE__);
	}
	else if(date("w", TIMENOW) == 6){//一周的最后一天
		$weekLogNum =  get_row_count("users","WHERE last_access >= ". sqlesc(date("Y-m-d H:i:s",(TIMENOW - 3849216))));
		sql_query('INSERT INTO everydaycount (LogTime, dayLogNum, weekLogNum,seedbounsNum, invitesNum)  VALUES (now(), '.$dayLogNum.', '.$weekLogNum.', '.$bonussums . ', ' . $invitesums . ')')or sqlerr(__FILE__, __LINE__);
	}
	else if(date("j", TIMENOW) == date("t", date("m", TIMENOW))){//一个月的最后一天
		$monthLogNum = get_row_count("users","WHERE last_access >= ". sqlesc(date("Y-m-d H:i:s",(TIMENOW - 86400 *date("t", date("m", TIMENOW) )))));
		sql_query('INSERT INTO everydaycount (LogTime, dayLogNum, monthLogNum,seedbounsNum, invitesNum)  VALUES (now(), '.$dayLogNum.', '.$monthLogNum.', '.$bonussums . ', ' . $invitesums . ')')or sqlerr(__FILE__, __LINE__);
	}
	else{//普通的一天
			sql_query('INSERT INTO everydaycount (LogTime, dayLogNum, seedbounsNum, invitesNum)  VALUES (now(), '.$dayLogNum.', '.$bonussums . ', ' . $invitesums . ')')or sqlerr(__FILE__, __LINE__);
	}
}
...
{
	sql_query("UPDATE users SET salarynum = 0 WHERE id = $row[id]") or sqlerr(__FILE__,__LINE__);
	//write_log("系统qinglile yonghu salrty--- $row[id]",'normal');
}

if ($printProgress) {
	printProgress("将超过3天未登录用户重置salarynum");
}
//自动清理过期的回收站、候选区种子
...
function user_to_peasant($down_floor_gb, $minratio){
	...
	if (mysql_num_rows($res) > 0)
	{
		while ($arr = mysql_fetch_assoc($res))
		{
			...
			writeBonusComment($arr['id'],"即将被封号，系统自动将所有麦粒（{$arr['seedbonus']}）换成上传量.");
...
$postIdHalfYearAgo=0;	
	...
	//sendMessage(0,2260,'所有自动清理完美',"autoclean----befor 换上传n----".date());
