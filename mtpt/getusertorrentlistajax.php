...
{
	global $lang_getusertorrentlistajax,$CURUSER,$smalldescription_main,$totalseedingsize;
	switch ($mode)
	{
		case 'uploaded': {
		$showsize = true;
...
if ($count){
	...
	if($type=='seeding'){
		sql_query("UPDATE users SET totalseed=$totalseedingsize WHERE id =$id") or sqlerr();
		echo "<b>".$count."</b>".$lang_getusertorrentlistajax['text_record'].add_s($count)."&nbsp;&nbsp;总保种体积：<b>".mksize_compact_totalseeding($totalseedingsize)."</b><br />".$torrentlist;
	}else{
		echo "<b>".$count."</b>".$lang_getusertorrentlistajax['text_record'].add_s($count)."<br />".$torrentlist;
	}
}
...
{
	sql_query("UPDATE users SET totalseed=0 WHERE id =$id") or sqlerr();
