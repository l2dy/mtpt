...
if (!$action) {
	...
	print("<tr><td class=rowfollow align=right colspan=5><input type=\"submit\" name=\"setdealt\" value=\"".$lang_staffbox['submit_set_answered']."\" /><input type=\"submit\" name=\"delete\" value=\"".$lang_staffbox['submit_delete']."\" /><input class=btn type=\"submit\" name=\"deletebonus\" value=\"删除求邀信息\" onclick=\"return confirm('确定删除？会将所有已读的求邀信息删除');\"/></td></tr>");
	print("</table>\n");
	print("</form>");
	echo $pagerbottom;
	end_main_frame();
...
if($_POST['deletebonus']){
	sql_query("DELETE FROM staffmessages WHERE subject='有游客申请邀请码' AND goto='0'") or sqlerr(__FILE__,__LINE__);
