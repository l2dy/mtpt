...
elseif ($row['banned'] == 'yes' && get_user_class() < $seebanned_class && !$owned)
	...
	if (!isset($_GET["cmtpage"])) {
		...
		if ($_GET["uploaded"])
		{
			...
			header("refresh: 1; url=download.php?id=$id&passkey=$passkey");
		...
		if ($CURUSER["downloadpos"] != "no")
		{
			...
			print("<a class=\"index\" href=\"download.php?id=$id&passkey=$passkey\">" . htmlspecialchars($torrentnameprefix ."." .$row["save_as"]) . ".torrent</a>&nbsp;&nbsp;&nbsp;".$lang_details['row_upped_by']."&nbsp;".$uprow.$uploadtime);
			print("</td></tr>");
		...
		if($row["ismttv"] == "yes"){
			$ismttv = "是";
		}elseif($row["ismttv"] =="no"){
			$ismttv = "否";
		}
        tr("是否MTTV组资源", $ismttv, 1);
        $prohibit_reshipment_texts = array(
            'no_restrain' => '<span style="color: green">无限制</span>',
            'cernet_only' => '<span style="color: indigo; font-weight: bold">仅限教育网转载</span>',
            'prohibited' => '<span style="color: red; font-weight: bold">禁止转载</span>'
		  );
        tr("转载限制", $prohibit_reshipment_texts[$row['prohibit_reshipment']], 1);
		if ($CURUSER['seedbonus'] < -1000) $bonusnotic = $lang_details['text_bonusnotic'];
		if ($CURUSER["downloadpos"] != "no")
			$download = "<a title=\"".$lang_details['title_download_torrent']."\" href=\"download.php?id=".$id."&passkey=".$passkey."\"><img class=\"dt_download\" src=\"pic/trans.gif\" alt=\"download\" />&nbsp;<b><font class=\"small\">".$lang_details['text_download_torrent'].$bonusnotic."</font></b></a>&nbsp;|&nbsp;";
		else $download = "";
		...
		print("<td class=\"embedded\"><form method=\"get\" action=\"http://sub.makedie.me/sub/\" target=\"_blank\"><input type=\"text\" name=\"searchword\" id=\"keyword\" style=\"width: 250px\" value=\"".$moviename."\" /><input type=\"submit\" value=\"".$lang_details['submit_search_at_shooter']."\" /></form></td><td class=\"embedded\"><form method=\"get\" action=\"http://www.zimuku.net/search\" target=\"_blank\"><input type=\"hidden\" id=\"q\" name=\"q\" /><input type=\"hidden\" name=\"action\"  /><input onclick=\"document.getElementById('moviename').value=document.getElementById('keyword').value;\" type=\"submit\" value=\"".$lang_details['submit_search_at_zimuku']."\" /></form></td>\n");
		print("</tr></table>");
		print("</td></tr>\n");
		// ---------------- end subtitle block -------------------//
	...
	if ($douban_id && $showextinfo['imdb'] == 'yes' && $CURUSER['showimdb'] != 'no')
	{
		...
		if(file_exists("./imdb/images/".$douban_id.".jpg")){
			$doubaninfo .= "<img src=\""."imdb/images/".$douban_id.".jpg"."\" style=\"max-width:100px\">";
		}
...
print("<br /><br />");
print ("<table style='border:1px solid #000000;'><tr><td class=\"text\" align=\"center\"><b>".$lang_details['text_quick_comment']."</b><br /><b style='color:red' >评论严禁：纯表情、求免费、求字幕 等无意义内容<br />up主如觉评论不适请举报</b><br /><form id=\"compose\" name=\"comment\" method=\"post\" action=\"".htmlspecialchars("comment.php?action=add&type=torrent")."\" onsubmit=\"return postvalid(this);\"><input type=\"hidden\" name=\"pid\" value=\"".$id."\" /><br />");
