  ...
  break;
		case "tracker":
			...
			function browsecheck($dbtable = "categories", $cbname = "cat"){
				...
				$sbnum = ($_POST["sbnum"] ? max(10, min(500, 0 + $_POST["sbnum"])) : 200);		
...
if ($showaudiocodec) $audiocodecs = searchbox_item_list("audiocodecs");
	...
	elseif (strpos($CURUSER['notifs'], "[spstate=6]") !== false)
		$special_state = 6;
			...
			$ss_r = sql_query("SELECT * FROM stylesheets ORDER BY id") or die;
			$ss_sa = array();
...
//显示学号验证状态
if ($cardnum != null){
	$cardconfirm = $CURUSER['cardconfirm'];
	if($cardconfirm == 'no') $cardconfirmtext = "您没有验证学号";
	if($cardconfirm == 'yes') $cardconfirmtext = "学号已验证";
	tr_small('学号状态', $cardconfirmtext, 1);
}
