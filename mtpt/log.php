...
<?php
	...
	$bonussums = get_row_sum("users","seedbonus");
	$invitesums = get_row_sum("users","invites");
?>
<tr>
<?php
...
<tr>
<?php
	twotd('全站麦粒总数',$bonussums);
	twotd('全站邀请码数',$invitesums);
?>
</tr>
<?php
	$Cache->end_whole_row();
...
<tr>
<?php
	twotd('MTTV 资源', get_row_count("torrents", "WHERE ismttv = 'yes'"));
	twotd('', '')
?>
</tr>
<?php
...
// ------------- end: stats ------------------//
	...
	case "funbox":
		...
		$res = sql_query("SELECT added, body, title, status, comment FROM fun $wherea ORDER BY added DESC $limit") or sqlerr(__FILE__, __LINE__);
		if (mysql_num_rows($res) == 0)
