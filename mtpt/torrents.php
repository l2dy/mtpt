...
if (get_user_class() < UC_DOWNLOADER){
	...
	{
		stdmsg("你访问了假麦田", "<h1>您使用IPV4地址访问了假麦田,使用假麦田会对正常麦田用户造成偷跑锐捷流量等各种不好的影响,所以该地址将在近期被禁用,使用该地址你无法下载本站的种子！<br/><b>请认准麦田PT官方地址：(pt.nwsuaf6.edu.cn)复制括号中的内容到浏览器地址栏粘贴访问。</b><br/>麦田PT只支持IPV6访问，西农用户请使用锐捷有线认证在宿舍及部分办公区获取IPV6地址。<br/>新手QQ群：145543216 麦田PT新人指导</h1>");
		stdfoot();
		exit;
	}
}
