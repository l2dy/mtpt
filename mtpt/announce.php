...
// if ($passkey == '00000000000000000000000000000000') err("Cheating!Invalid passkey". " - $passkey"); //因sqlesc函数只对字符加单引号，导致32个0任意下载漏洞，需要加上这行修复。2017.05.21纳兰斯坦
// if ($passkey == '00000000000000000000000000000000') err("Cheating!Invalid passkey"); //因sqlesc函数只对字符加单引号，导致32个0任意下载漏洞，需要加上这行修复。直接修改了sqlesc函数，此行可注释2017.05.21纳兰斯坦
...
if (!ip2long($ip)) //Disable compact announce with IPv6
	$compact = 0;
//if (!ip_filter($ip,true,true,$bannedV6,$bannedV4)) err("invalid ip"); //屏蔽ipv4地址和部分ipv6地址,函数和屏蔽数组扔在include/function_announce.php
...
else // continue an existing session
{
	...
	$upseedbonus = $trueupthis/1024 * 0.0000006;		//上传影响积分，单位每K
